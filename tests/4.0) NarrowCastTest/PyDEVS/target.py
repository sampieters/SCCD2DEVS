...
Model description:
Test 12: Check if an instance narrowcast properly
"""
...
class MainAppInstance(RuntimeClassBase):
    ...
    def _state4_enter(self):
        self.big_step.outputEventOM(Event("narrow_cast", None, [self, 'linkA[1]', Event("test_narrow", None, [])]))
    ...
    def _state1_0_exec(self, parameters):
        ...
        self.big_step.outputEventOM(Event("start_instance", None, [self, "linkA[0]"]))
    
    def _state2_0_exec(self, parameters):
        association_name = parameters[0]
        self.big_step.outputEvent(Event("instance_created_succesfully", self.getOutPortName("ui"), [association_name]))
        self.big_step.outputEventOM(Event("start_instance", None, [self, "linkA[1]"]))
    
    def _state3_0_exec(self, parameters):
        association_name = parameters[0]
        self.big_step.outputEvent(Event("instance_created_succesfully", self.getOutPortName("ui"), [association_name]))
        self.big_step.outputEventOM(Event("start_instance", None, [self, "linkA[2]"]))
...
class AInstance(RuntimeClassBase):
    ...
    def build_statechart_structure(self):
        ...
        _state1_0.setAction(self._state1_0_exec)
        _state1_0.setTrigger(Event("test_narrow", None))
        self.states["/state1"].addTransition(_state1_0)
    
    def _state1_0_exec(self, parameters):
        self.big_step.outputEvent(Event("narrowcast_received", self.getOutPortName("ui"), []))
