...
class AInstance(RuntimeClassBase):
    ...
    def build_statechart_structure(self):
        ...
        # state /not_possible
        self.states["/not_possible"] = State(2, "/not_possible", self)
        self.states["/not_possible"].setEnter(self._not_possible_enter)
        
        # add children
        self.states[""].addChild(self.states["/state1"])
        self.states[""].addChild(self.states["/not_possible"])
        self.states[""].fixTree()
        self.states[""].default_state = self.states["/state1"]
        
        # transition /state1
        _state1_0 = Transition(self, self.states["/state1"], [self.states["/not_possible"]])
        _state1_0.setTrigger(Event("instance_created", None))
        self.states["/state1"].addTransition(_state1_0)
    ...
    def _not_possible_enter(self):
        self.big_step.outputEvent(Event("should_not_happen", self.getOutPortName("ui"), []))
